...
type Props = {
  abilities: Ability[];
  points: number;
  useAbility: (id: number) => void;
};

export function SpecialAbilities({ abilities, points, useAbility }: Props) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {abilities.map((ability) => {
        ...
        return (
          <div
            key={ability.id}
            className="bg-gray-800 bg-opacity-50 p-4 rounded-lg shadow-lg relative overflow-hidden"
          >
            {/* Cooldown overlay */}
            {cooldownRemaining > 0 && (
            ...
            {/* Active indicator */}
            {ability.active && (
              <div className="absolute inset-0 border-2 border-yellow-400 animate-pulse rounded-lg pointer-events-none"></div>
            )}
