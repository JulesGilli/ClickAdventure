...
export function UpgradeShop({ upgrades, points, purchaseUpgrade }: Props) {
  const upgradeItems: {
    type: UpgradeType;
    name: string;
    description: string;
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {upgradeItems.map((item) => {
        ...
        return (
          <div
            key={item.type}
            className="bg-gray-800 bg-opacity-50 p-4 rounded-lg shadow-lg"
          >
            <div className="flex items-center mb-3">
              ...
              <div>
                <h3 className="font-bold">
                  {item.name} <span className="text-sm text-yellow-400">T{upgrade.tier}</span>
                </h3>
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => purchaseUpgrade(item.type)}
                disabled={!canAfford}
